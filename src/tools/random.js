  ...
  parameters: {
    ...
    properties: {
      type: {
        type: 'string',
        description:
          'Type of random selection: "choice" (pick from list), "number" (random number), "coin" (flip coin), "dice" (roll dice)',
      },
      options: {
        type: 'array',
        description:
          'For type "choice": array of options to choose from (e.g., ["pizza", "burger", "sushi"])',
        items: { type: 'string' },
      },
      min: {
        type: 'number',
      ...
      switch (type.toLowerCase()) {
        ...
        case 'coin': {
          ...
          return {
            ...
            summary:
              numSelections === 1 ? `Coin flip: ${flips[0]}` : `Coin flips: ${flips.join(', ')}`,
          };
        }
